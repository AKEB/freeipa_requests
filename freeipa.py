    ...
    def add_user_to_group(self, groups: list = []) -> None:
        ...
        for group in groups:
            ...
            if not result or ('failed' in result and 'completed' in result and int(result['completed']) < 1):
                if result and ('failed' in result and 'member' in result['failed'] and 'user' in result['failed']['member'] and len(result['failed']['member']['user']) > 0 and len(result['failed']['member']['user'][0]) > 1):
